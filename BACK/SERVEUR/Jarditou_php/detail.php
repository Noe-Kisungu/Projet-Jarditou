...
<img src="images/jarditou_photos/<?=$produit->pro_id?>" width="300" alt="produit"> <!-- Pour ajouter la photo du produit : width="300 permet de redimensionner la photo et en n'indiquant qu'un seul paramètre le navigateur se charge de calculer le deuxième c'est à dire height en conservant les proportions de départ -->
</div>
...
<!-- <a href="/php/produits_modif_script.php?id=<?php echo $r['pro_id']?>" class="btn btn-warning">EDITER</a> -->
    ...
    //     {
        ...
        <!-- Quand on clique sur le bouton modifier on exécute le script du fichier sur lequel on fait un lien et on récupère l'ID avec ?pro_id=<?= $produit->pro_id?> -->
         <a href="produits_modif.php?pro_id=<?= $produit->pro_id?>" class="btn btn-info m-0">Modifier</a>
        ...
        <!-- Quand on clique sur le bouton Ajouter dans mon panier on exécute le script du fichier sur lequel on fait un lien et on récupère l'ID avec ?pro_id=<?= $produit->pro_id?> -->
        <a href="panier.php?action=ajout&l=<?=$produit->pro_libelle?>&amp;q=1&p=<?=$produit->pro_prix?>" class="btn btn-warning">Ajouter au panier</a>
        <?php
        // }
    ?>
</div>
