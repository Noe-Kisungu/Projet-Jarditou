...
SELECT      CompanyName AS "Client", SUM(UnitPrice*Quantity) AS "CA", Country AS "Pays" FROM orderdetails, customers, orders
...
SELECT  customers.country AS "Pays" FROM customers, orders, orderdetails, products, suppliers
WHERE   customers.customerID = orders.customerID 
AND     orders.orderID = orderdetails.orderId 
...
SELECT  SUM(UnitPrice*Quantity) AS "montant ventes 97" FROM orderdetails, orders
WHERE   orderdetails.OrderID = orders.OrderID 
AND     YEAR(OrderDate) = 1997
-- 8 - Montant des ventes de 1997 mois par mois :
SELECT      MONTH(Orderdate) AS "Mois_97", SUM(UnitPrice*Quantity) AS "montant_ventes_97" FROM orderdetails, orders
WHERE       orderdetails.OrderID = orders.OrderID 
AND         YEAR(OrderDate) = 1997 
GROUP BY    Mois_97
